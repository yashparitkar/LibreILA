...
if (AXI4_STREAM_IF != 0)
    begin
        `ifdef TEST_CASE_0 // Stream descriptor bridging support
...
`ifdef TEST_CASE_1 // External descriptor fetch
    ...
    // is complete whilst waiting on the Control INITIATOR to service the interrupt.
    config_int_dscrptr(
        DMA_0_BASE_ADDR, // Base addresss of CoreAXI4DMAController instance
...
`ifdef TEST_CASE_2 // Simple DMA transfer with narrow last beat
...
`ifdef TEST_CASE_3 // Large DMA transfer requiring multiple AXI transactions and spanning 4 KB boundary on dest side
...
`ifdef TEST_CASE_4 // Internal descriptor chain operation
...
`endif
`ifdef TEST_CASE_5 // Multiple coincident start bits for same priority level descriptors
