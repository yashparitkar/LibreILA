...
% Last Modified: 2026-08-11 Tue 15:46
%
...
\section{Description}
LibreILA is an in-system \acf{ILA} written in plain VHDL. It is spliced into a link inside the \acs{FPGA} fabric and samples that link into a circular buffer held in fabric RAM, so nets that never reach a pad are as visible as the ones that do. It exists because the Microchip toolchain has no equivalent of the Xilinx \acs{ILA} and SmartDebug's live probes are not a substitute for a triggered, time-correlated capture.

The core is deliberately ignorant of what it observes: one flat probe word of \texttt{G\_PROBE\_WIDTH} bits, whose bit order the trigger, the sample buffer and both drivers all inherit. A build is therefore described entirely by two CSV files. Two deliverables are provided: \texttt{libre\_ila}, the bare core with an AXI4Lite slave port and a baremetal C driver, and \texttt{libre\_ila\_uart}, a serial wrapper driven from a PC by the python driver. The baremetal path is the differentiator: capture and readout need no host on the wire at all.
...
Edge detection is applied to the reduced condition, not per probe bit, because by that point the whole probe word has already been reduced to one bit and the mode costs a single flip-flop. Asking for ``bit 5 rises \emph{and} bit 9 falls in the same sample'' is therefore not expressible.
...
Readout needs no synchroniser at all. The sample buffer is a dual-port \acs{LSRAM} whose read port is clocked by the AXI4Lite clock, which is what the PolarFire \acs{LSRAM} independent clock port exists for.

\subsection{Readout}
Samples are stored one 32-bit lane per slice of the probe word, so lane $n$ carries probe bits $32n+31$ downto $32n$ and the last lane is zero padded in its upper bits. On readout the lanes are padded further, up to \texttt{C\_AXIL\_STRIDE} registers per sample, so that a sample index maps onto an address with a shift instead of a multiply. The trigger vector uses that same padded layout, which is what makes a captured sample and a trigger condition directly comparable word for word.
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        \hline
        \texttt{G\_SAMP\_CLK\_FREQ} & 100\,000\,000 & Sampling clock frequency in Hz. Reported over AXI4Lite so the host can build the time axis; it configures no logic. \\
        \hline
        \texttt{G\_AXIL\_CLK\_FREQ} & 100\,000\,000 & AXI4Lite clock frequency in Hz. Sets the wrapper's watchdog period and the \acs{UART} divider. \\
        \hline
        \texttt{G\_EXTERNAL\_TRIG} & 0 & 1 uses the \texttt{i\_ext\_trig} pin instead of the trigger vector. \\
        \hline
        \texttt{G\_PROBE\_WIDTH} & 67 & Total probed bits. Keep it a multiple of 32 for best results. Must be greater than zero, checked at elaboration. \\
        \hline
        \texttt{G\_SAMP\_BUFF\_DEPTH} & 2048 & Number of samples stored. Must be a power of two greater than one, checked at elaboration. \\
        \hline
        \texttt{G\_UID} & 0 & Identity of this instance, reported at UID so that a system holding several cores can tell them apart. Configures no logic and every value is legal, zero meaning unset. A \texttt{natural}, so at most \hex{7FFFFFFF}. \\
        \hline
        \texttt{G\_UART\_RX\_FIFO\_DEPTH} & 1024 & Wrapper only. Depth of the \acs{UART} receive FIFO in bytes. \\
        \hline
        \texttt{G\_UART\_TX\_FIFO\_DEPTH} & 1024 & Wrapper only. Depth of the \acs{UART} transmit FIFO in bytes. \\
        \hline
        \texttt{G\_BAUD\_RATE} & 115\,200 & Wrapper only. \acs{UART} baud rate. \\
        \hline
        \texttt{C\_S\_AXIL\_DATA\_WIDTH} & 32 & AXI4Lite data width. Do not change. \\
        \hline
        \texttt{C\_S\_AXIL\_ADDR\_WIDTH} & 32 & AXI4Lite address width. Do not change. \\
        \hline
    \end{tabularx}
...
Left at their shipped defaults, the two files describe the stock 64-bit AXI4Stream build. \texttt{GEN\_TYPE} in the configuration selects what comes out: 0 emits the bare AXI4Lite core alone, 1 emits it together with the \acs{UART} wrapper and the FIFO and \acs{UART} blocks it instantiates. Add the emitted files to the project, instantiate the core or the wrapper, and splice the probe ports into the link to be observed. The full procedure is in the user manual; this section is a quick glance.

\todo{Add screenshots of the generator run and of the core instantiated in SmartDesign once the core is released.}
...
The constant is the same in every build, deliberately: it identifies the core \emph{type}, never the individual core. Telling one instance from another is the job of UID at \hex{14}, and the two are kept apart so that a host can still recognise a core whose UID it has never been told.
...
Splitting the two is what makes discovery work. A host walking a list of base addresses, or of serial ports, checks MGCKEY to know a core is there at all and only then reads UID to find out which one it is. Were the identity folded into MGCKEY instead, a core whose \texttt{G\_UID} the host had not been told in advance would be indistinguishable from a peripheral that is not an ILA, or from a bus returning zeros.

For that reason every value is legal and none is validated, by the hardware or by either driver. Zero is what a core built without a \texttt{G\_UID} reports and means no more than \emph{unset}.
...
That padding is what keeps addressing cheap: with $a$ a power of two, a sample index becomes an address with a shift instead of a multiply. It costs bus address space and not RAM, since the buffer itself is only \texttt{C\_N\_LANES} lanes wide.
...
\texttt{libre\_ila\_uart} wraps the core with a \acs{UART} and a small packet parser, so that a design with no processor, or with no spare AXI master, can still be debugged from a PC over a two-wire serial link. The wrapper instantiates the core and carries all of its generics, adding the FIFO depths and the baud rate. The register map seen from the PC is exactly the map of Table~\ref{tab:regsummary}, reached by address rather than through any command protocol.
...
\todo{Add the GUI subsection here once the GUI is released, with screenshots of a capture being configured and read back.}
...
\begin{itemize}
    \item \textbf{Repository and full documentation} ---
          \url{github.com/yashparitkar/libreila}. The top level README is the authoritative description of the register map, the trigger vector and the packet format.
...
Table~\ref{tab:utilization-core} isolates the \texttt{libre\_ila} core itself (the sample buffer, trigger comparator, address decode and AXI4Lite slave state machine), the \acs{UART} wrapper that surrounds it in the \acs{IP} used for this measurement.
